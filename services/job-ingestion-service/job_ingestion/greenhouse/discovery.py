"""Finding a company's Greenhouse board without anyone typing its name.
...
Verification, which is the half worth building. A Greenhouse board states the
company it belongs to on every posting it returns, so a guess can be checked
against the company it was derived from before a single record is ingested. A
slug that resolves to somebody else is rejected and reported, not stored.
...
if TYPE_CHECKING:
    from job_ingestion.greenhouse.client import GreenhouseClient
...
async def discover(client: "GreenhouseClient", company_name: str) -> DiscoveryResult:
...
def _stated_company(page: RawPage) -> str | None:
    """The company a board says its postings belong to.

    Read from the records rather than trusted from the slug. An empty board
    states nothing, and a board that states nothing cannot confirm anything.
    """
