"""Scheduled Jobicy job ingestion.
...
from job_ingestion.jobicy import ingest_jobicy

START_DATE = datetime(2026, 1, 1)
# Offset from Arbeitnow's :00 so the two providers do not compete for the
# same minute. Hourly is the documented ceiling Jobicy asks for.
SCHEDULE = "15 * * * *"
MAX_RECORDS = 500
...
@dag(
    ...
    description="Fetch Jobicy postings into the canonical job catalog",
    schedule=SCHEDULE,
...
def jobicy_ingestion() -> None:
    ...
    def ingest() -> dict[str, int | str]:
        ...
        """
        summary = asyncio.run(ingest_jobicy(max_records=MAX_RECORDS))
        return {
